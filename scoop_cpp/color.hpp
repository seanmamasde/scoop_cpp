// Modified from: https://github.com/kkettinger/color-console/blob/fix-linker-errors/include/color.hpp
// which is modified from: https://github.com/aafulei/color-console/blob/master/include/color.hpp
// and could be found on vcpkg.
// However, linker will throw an error if it's used in multiple translation units,
// since it does not have the `inline` keyword.
//
// Changes:
// - Changed header guards to `#pragma once` (for uniformity of this project).
// - Changed the inheritance of `colorful` from private of `std::list` to public.
// - Added an initializer for `colorful<std::string>` so that it takes in a string and preserve its original color.
// - Added comparison operators overload for `colorful<std::string>`.
...
namespace dye
{
    ...
    template<typename T>
    class colorful : public std::list<item<T>>
    {
    public:
        using std::list<item<T>>::list;

        // Constructor that accepts a std::string and converts it to colorful<string>
        explicit colorful(const std::string& str)
        {
            this->emplace_back(str, hue::get_text()); // Preserve current text color
        }
        ...
        // Utility function to get the concatenated underlying string
        std::string to_string() const
        {
            std::string concatenated;
            for (const auto& elem : *this)
            {
                concatenated += elem.thing;
            }
            return concatenated;
        }

        // Comparison operator overloads
        bool operator==(const colorful<T>& other) const
        {
